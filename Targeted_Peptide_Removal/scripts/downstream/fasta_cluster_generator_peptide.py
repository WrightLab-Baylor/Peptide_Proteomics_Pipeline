...
def find_default_input():
    """
    Find the single FASTA/FAA input in the *working directory* (same level as ./database/).

    Returns:
      - str path to the FASTA/FAA if exactly one is found
      - None if none found (caller should "end early")

    Notes:
      - We intentionally do NOT search ./database/ so that peptide FASTAs can live there
        while this script clusters the protein FASTA placed in the working directory.
    """
    fasta_files = glob("*.fasta") + glob("*.faa")
    if len(fasta_files) == 0:
        return None
    elif len(fasta_files) > 1:
        raise RuntimeError("❌ Multiple FASTA/FAA files found in the working directory. Please use -i to specify input.")
...
def resolve_cdhit_path(provided_path=None):
    ...
    if found:
        return found
    sys.exit("[ERROR] cd-hit binary not found in PATH. Install cd-hit or pass --cdhit-path.")
...
def main():
    ...
    parser.add_argument("-i", "--input", help="Input FASTA/FAA file (default: single *.fasta/*.faa in working directory)", default=None)
    ...
    parser.add_argument("--cdhit-path", type=str, default=None, help="Optional explicit path to the cd-hit executable")
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Directory to move the ORIGINAL (unclustered) FASTA after clustering. "
            "Default: ./Unclustered_FASTA (only when clustering is performed)."
        ),
    )
    ...
    if not input_fasta:
        print("[INFO] No FASTA/FAA found in working directory; nothing to cluster. Exiting.")
        return
    ...
    # Existing cluster annotations satisfy this step; only require cd-hit when
    # clustering actually needs to be performed.
    if is_already_processed(input_fasta):
        print(f"[INFO] File '{input_fasta}' already contains cluster annotations. Exiting.")
        return
    ...
    # Move original FASTA out of the way ONLY if clustering was performed
    if clustered_ran:
        dest_dir = args.move_to
        if not dest_dir:
            dest_dir = os.path.join(os.getcwd(), "Unclustered_FASTA")
        move_original_fasta(input_fasta, dest_dir)
