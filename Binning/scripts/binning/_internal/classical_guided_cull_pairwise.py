#!/usr/bin/env python3
"""
Internal Pairwise-bin engine for classical-guided SIC culling.

This module is used by ``scripts/binning/classical_guided_cull.py`` when the
Pairwise-bin workflow is selected. It is not intended to be the primary
user-facing entry point.

For each Pairwise-bin folder, the engine retains SIC rows whose
...
for a reference scan that was not recovered from SICs in that pair.

Key behavior
------------
- SIC-derived rows take precedence over SYN fallback rows.
- SYN fallback contributes at most one representative row per scan per pair.
...
Primary outputs
---------------
Each Pairwise-bin folder receives a classical-guided SIC table under the
...
def parse_pair_groups(run_name: str) -> Optional[Tuple[int, int]]:
    match = PAIR_RE.match(run_name)
    if not match:
        return None
...
def find_pair_folders(dataset_dir: Path) -> List[Path]:
    pair_folders = sorted(
        p for p in dataset_dir.glob("*_pair_*")
        if p.is_dir() and parse_pair_groups(p.name) is not None
    )
    if not pair_folders:
        raise RuntimeError(f"No pair folders matching '*_pair_i_j' found in {dataset_dir}")
    return pair_folders
...
def load_reference_withsyn(
    ...
    reference_pairs = (
        reference_pairs.groupby(["ScanNum", "CorePeptide"], as_index=False)
        .agg(
    ...
    allowed_pairs = set(zip(reference_pairs["ScanNum"], reference_pairs["CorePeptide"]))

    reference_scan_to_peptides: Dict[object, Set[str]] = defaultdict(set)
    for scan, pep in zip(reference_pairs["ScanNum"], reference_pairs["CorePeptide"]):
        reference_scan_to_peptides[scan].add(pep)

    scan_summary = (
        reference_pairs.groupby("ScanNum", as_index=False)
        .agg(
    ...
    return allowed_pairs, reference_pairs, scan_summary, reference_scan_to_peptides


def load_sic_file(path: Path, scan_col: str, peptide_col: str, protein_col: str) -> pd.DataFrame:
    df = pd.read_csv(path, sep="\t")
    require_columns(df, {scan_col, peptide_col, protein_col}, path)
    df = df.copy()
...
def find_syn_file(pair_folder: Path, syn_dir_name: str, syn_glob: str) -> Optional[Path]:
    syn_dir = pair_folder / syn_dir_name
...
def load_syn_file(
    ...
    The pair-level SYN file may not have exactly the same columns as PlusSICStats.
...
def choose_one_syn_fallback_per_scan(
    ...
    allowed_pairs: Set[Tuple[object, str]],
    scan_col: str,
    peptide_col: str,
) -> pd.DataFrame:
    """
    ...
    We only fill scans that are absent from retained SIC rows in the same pair.
...
def scan_category_counts(
    ...
    pair_scan_to_peptides: Dict[object, Set[str]] = defaultdict(set)
    for scan, pep in zip(file_df[scan_col], file_df["CorePeptide"]):
        if pep:
            pair_scan_to_peptides[scan].add(pep)
    ...
    pair_scans = set(pair_scan_to_peptides)
    reference_scans = set(reference_scan_to_peptides)

    scans_in_reference_and_pair = pair_scans & reference_scans
    alternative_only = sum(1 for scan in scans_in_reference_and_pair if scan not in recovered_scans)

    return {
        "PairUniqueScans": len(pair_scans),
        ...
        "PairScansNotInReference": len(pair_scans - reference_scans),
    }


def build_recovery_outputs(
    reference_pairs: pd.DataFrame,
    recovered_scan_to_pairs: Dict[object, Set[str]],
    sic_recovered_scan_to_pairs: Dict[object, Set[str]],
    syn_recovered_scan_to_pairs: Dict[object, Set[str]],
    syn_observed_scan_to_pairs: Dict[object, Set[str]],
) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """Create global recovery tables from all pair-level kept rows."""
    ...
    for scan, pep, is_decoy in zip(
        reference_pairs["ScanNum"],
        reference_pairs["CorePeptide"],
        reference_pairs["ReferenceIsDecoy"],
    ):
    ...
    for scan in sorted(ref_scan_to_peptides):
        all_pairs = sorted(recovered_scan_to_pairs.get(scan, set()))
        sic_pairs = sorted(sic_recovered_scan_to_pairs.get(scan, set()))
        syn_added_pairs = sorted(syn_recovered_scan_to_pairs.get(scan, set()))
        syn_observed_pairs = sorted(syn_observed_scan_to_pairs.get(scan, set()))
        rows.append({
            "ScanNum": scan,
            "ReferenceCorePeptides": ";".join(sorted(ref_scan_to_peptides[scan])),
            "ReferenceHasDecoy": ref_scan_to_decoy[scan],
            "RecoveredInAnyPair": len(all_pairs) > 0,
            "RecoveredPairCount": len(all_pairs),
            "RecoveredPairFolders": ";".join(all_pairs),
            "SICRecoveredPairCount": len(sic_pairs),
            "SICRecoveredPairFolders": ";".join(sic_pairs),
            "SYNFallbackAddedPairCount": len(syn_added_pairs),
            "SYNFallbackAddedPairFolders": ";".join(syn_added_pairs),
            "SYNObservedMatchingPairCount": len(syn_observed_pairs),
            "SYNObservedMatchingPairFolders": ";".join(syn_observed_pairs),
            "RecoveredBySIC": len(sic_pairs) > 0,
            "RecoveredBySYNFallback": len(syn_added_pairs) > 0,
            "ObservedInSYNAtLeastOnce": len(syn_observed_pairs) > 0,
        })

    recovery_by_scan = pd.DataFrame(rows)
    unrecovered = recovery_by_scan[~recovery_by_scan["RecoveredInAnyPair"]].copy()

    total_reference_scans = len(recovery_by_scan)
    recovered_scans = int(recovery_by_scan["RecoveredInAnyPair"].sum()) if total_reference_scans else 0
    ...
    summary = pd.DataFrame([
        ...
        {"Metric": "RecoveredInAtLeastOnePair", "Value": recovered_scans},
        {"Metric": "RecoveredBySICInAtLeastOnePair", "Value": sic_recovered_scans},
        {"Metric": "RecoveredBySYNFallbackInAtLeastOnePair", "Value": syn_fallback_recovered_scans},
        {"Metric": "ObservedMatchingSYNInAtLeastOnePair", "Value": syn_observed_scans},
        {"Metric": "UnrecoveredInAllPairs", "Value": unrecovered_scans},
        {"Metric": "RecoveredFraction", "Value": recovery_fraction},
        {"Metric": "RecoveredPairCountDistribution", "Value": distribution("RecoveredPairCount")},
        {"Metric": "SICRecoveredPairCountDistribution", "Value": distribution("SICRecoveredPairCount")},
        {"Metric": "SYNFallbackAddedPairCountDistribution", "Value": distribution("SYNFallbackAddedPairCount")},
        {"Metric": "SYNObservedMatchingPairCountDistribution", "Value": distribution("SYNObservedMatchingPairCount")},
    ])
...
def write_reference_outputs(
    ...
    ref_pairs_out = dataset_dir / "classical_guided_reference_scan_peptides.tsv"
    ref_summary_out = dataset_dir / "classical_guided_reference_scan_summary.tsv"

    if not dry_run:
        reference_pairs.to_csv(ref_pairs_out, sep="\t", index=False)
        scan_summary.to_csv(ref_summary_out, sep="\t", index=False)

    return ref_pairs_out, ref_summary_out
...
def cull_pair_files(
    ...
    pair_folders = find_pair_folders(dataset_dir)

    recovered_scan_to_pairs: Dict[object, Set[str]] = defaultdict(set)
    sic_recovered_scan_to_pairs: Dict[object, Set[str]] = defaultdict(set)
    syn_recovered_scan_to_pairs: Dict[object, Set[str]] = defaultdict(set)
    syn_observed_scan_to_pairs: Dict[object, Set[str]] = defaultdict(set)

    summary_rows: List[Dict[str, object]] = []
    audit_rows: List[Dict[str, object]] = []

    for pair_folder in pair_folders:
        pair_groups = parse_pair_groups(pair_folder.name)
        assert pair_groups is not None

        sic_dir = pair_folder / sic_dir_name
        if not sic_dir.is_dir():
            summary_rows.append({
                "RunFolder": pair_folder.name,
                "PairGroupA": pair_groups[0],
                "PairGroupB": pair_groups[1],
                "SourceFile": "",
                "SynFile": "",
        ...
        syn_file = find_syn_file(pair_folder, syn_dir_name=syn_dir_name, syn_glob=syn_glob) if use_syn_fallback else None
        ...
        if use_syn_fallback and syn_file is not None:
            try:
                ...
                for scan in syn_matching_all[scan_col].unique():
                    syn_observed_scan_to_pairs[scan].add(pair_folder.name)
        ...
        out_dir = pair_folder / out_dir_name
        if not dry_run:
        ...
        for sic_file in sic_files:
            ...
            for scan, source_layer in zip(combined[scan_col], combined.get("SourceLayer", pd.Series([""] * len(combined)))):
                recovered_scan_to_pairs[scan].add(pair_folder.name)
                if source_layer == "SYN":
                    syn_recovered_scan_to_pairs[scan].add(pair_folder.name)
                else:
                    sic_recovered_scan_to_pairs[scan].add(pair_folder.name)
            ...
            summary_rows.append({
                ...
                "DecoyRows": n_decoy,
                "RowFDR": row_fdr,
                "PairUniqueScans": scan_counts["PairUniqueScans"],
                ...
                "PairScansNotInReference": scan_counts["PairScansNotInReference"],
            ...
            for scan in sorted(pair_scan_to_peptides):
                ref_peps = reference_scan_to_peptides.get(scan, set())
                pair_peps = pair_scan_to_peptides[scan]
                if scan in kept_sic_scans_after:
                    ...
                    category = "PairScanNotInReference"
                audit_rows.append({
                    "RunFolder": pair_folder.name,
    ...
    recovery_by_scan, unrecovered, recovery_summary = build_recovery_outputs(
        ...
        recovered_scan_to_pairs=recovered_scan_to_pairs,
        sic_recovered_scan_to_pairs=sic_recovered_scan_to_pairs,
        syn_recovered_scan_to_pairs=syn_recovered_scan_to_pairs,
        syn_observed_scan_to_pairs=syn_observed_scan_to_pairs,
    )
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Internal Pairwise-bin culling engine. Use classical_guided_cull.py as the public entry point."
    )
    p.add_argument("--dataset_dir", required=True, type=Path,
                   help="Dataset directory containing Pairwise-bin folders named *_pair_i_j.")
    ...
    p.add_argument("--sic_dir_name", default="SICs",
                   help="Input SIC directory inside each pair folder. Default: SICs")
    p.add_argument("--sic_glob", default="*_PlusSICStats.tsv",
                   help="Glob for pair-level SIC files. Default: *_PlusSICStats.tsv")
    p.add_argument("--syn_dir_name", default="results/PHRPOut",
                   help="SYN directory inside each pair folder. Default: results/PHRPOut")
    p.add_argument("--syn_glob", default="*_syn.txt",
                   help="Glob for pair-level SYN files. Default: *_syn.txt")
    ...
    p.add_argument("--out_dir_name", default="SICs_classical_guided",
                   help="Output directory inside each pair folder. Default: SICs_classical_guided")
    ...
    p.add_argument("--dry_run", action="store_true",
                   help="Report what would be done without writing pair-level output files.")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    print("Classical-guided Pairwise-bin culling engine")
    print("=" * 60)
    ...
    allowed_pairs, reference_pairs, scan_summary, reference_scan_to_peptides = load_reference_withsyn(
    ...
    ref_pairs_out, ref_summary_out = write_reference_outputs(
    ...
    print(f"Reference rows loaded as unique ScanNum/CorePeptide pairs: {len(reference_pairs):,}")
    ...
    print(f"Reference peptide table: {ref_pairs_out}")
    print(f"Reference scan summary: {ref_summary_out}")
    print()

    print("Culling pair SIC files, adding SYN fallback rows where needed, and auditing recovery...")
    summary, audit, recovery_by_scan, unrecovered, recovery_summary = cull_pair_files(
        dataset_dir=dataset_dir,
        allowed_pairs=allowed_pairs,
        reference_pairs=reference_pairs,
        reference_scan_to_peptides=reference_scan_to_peptides,
    ...
    summary_out = dataset_dir / "classical_guided_pair_culling_summary.tsv"
    audit_out = dataset_dir / "classical_guided_pair_scan_audit.tsv"
    recovery_by_scan_out = dataset_dir / "classical_guided_reference_recovery_by_scan.tsv"
    unrecovered_out = dataset_dir / "classical_guided_reference_unrecovered_scans.tsv"
    recovery_summary_out = dataset_dir / "classical_guided_reference_recovery_summary.tsv"
    ...
    print(f"Pair summary: {summary_out}")
    print(f"Pair scan audit: {audit_out}")
    ...
    recovered_any = int(rec_metrics.get("RecoveredInAtLeastOnePair", 0))
    recovered_by_sic = int(rec_metrics.get("RecoveredBySICInAtLeastOnePair", 0))
    recovered_by_syn = int(rec_metrics.get("RecoveredBySYNFallbackInAtLeastOnePair", 0))
    syn_observed = int(rec_metrics.get("ObservedMatchingSYNInAtLeastOnePair", 0))
    unrecovered_count = int(rec_metrics.get("UnrecoveredInAllPairs", 0))
    recovered_fraction = float(rec_metrics.get("RecoveredFraction", float("nan")))
    recovery_distribution = rec_metrics.get("RecoveredPairCountDistribution", "")
    sic_distribution = rec_metrics.get("SICRecoveredPairCountDistribution", "")
    syn_added_distribution = rec_metrics.get("SYNFallbackAddedPairCountDistribution", "")
    syn_observed_distribution = rec_metrics.get("SYNObservedMatchingPairCountDistribution", "")

    print("Run summary")
    print("-" * 60)
    print(f"Pair files represented: {len(summary):,}")
    ...
    print(f"Recovered in >=1 pair:              {recovered_any:,}")
    print(f"Recovered by SIC in >=1 pair:       {recovered_by_sic:,}")
    print(f"Recovered by SYN fallback >=1 pair: {recovered_by_syn:,}")
    print(f"Observed matching SYN >=1 pair:     {syn_observed:,}")
    print(f"Unrecovered in all pairs:           {unrecovered_count:,}")
    ...
    print(f"Recovered pair count distribution:  {recovery_distribution}")
    print(f"SIC pair count distribution:        {sic_distribution}")
    print(f"SYN added pair count distribution:  {syn_added_distribution}")
    print(f"SYN observed pair count distribution:{syn_observed_distribution}")
    print("=" * 60)
    print("Done.")
